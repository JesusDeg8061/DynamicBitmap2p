   ...
   public void sendChunk(String host, int port, int index) {
    ...
    public void smartReplicate() {
        ...
        for (NodeInfo n : neighbors) {
            ...
            String remoteBitmap = requestBitmap(n.host, n.port);
            if (remoteBitmap == null) continue;

            int limit = Math.min(bitmap.getSize(), remoteBitmap.length());

            if (remoteBitmap.equals(lastKnownBitmaps.get(key))) continue;

            lastKnownBitmaps.put(key, remoteBitmap);

            System.out.println("Nodo " + id + ": nuevo bitmap detectado en " + n.port);

            for (int i = 0; i < limit; i++) {

                if (hasChunk(i) && remoteBitmap.charAt(i) == '0') {
                    System.out.println("Nodo " + id + " enviando chunk " + i + " a " + n.port);
                    sendChunkAsync(n.host, n.port, i);
                }
            }
        }
    }
